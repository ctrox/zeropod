...
var ErrListenersNotFound = errors.New("listeners not found")
...
func (act *Activator) registerListeners(pid int) error {
	...
	if len(listeners) < len(act.ports) {
		return fmt.Errorf("%w: expected at least %d listeners, found %d", ErrListenersNotFound, len(act.ports), len(listeners))
	}
...
func (act *Activator) wake(port uint16, network network) error {
...
func (act *Activator) ScaleDown() error {
	...
	for k := range act.wakeListeners {
		...
		if err := act.listen(k.port, k.network, act.wakeListeners[k]); err != nil {
			return err
		}
	}
...
func (act *Activator) listen(port uint16, network network, wl *wakeListener) error {
...
func (act *Activator) watchWake(f *os.File, port uint16, network network) {
	...
	for {
		...
		for i := range n {
			...
			if err := act.wake(port, network); err != nil {
