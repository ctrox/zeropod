...
func (r *migrationReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.kube.Get(ctx, request.NamespacedName, migration); err != nil {
		...
		return reconcile.Result{}, fmt.Errorf("getting migration %s", err)
	}
	// should be handled by the cache already but just to be sure
	if migration.Spec.SourceNode != r.nodeName {
		return reconcile.Result{}, nil
	}

	return reconcile.Result{}, r.handleImageCleanup(migration)
}

func (r *migrationReconciler) handleImageCleanup(migration *v1.Migration) error {
	for _, container := range migration.Status.Containers {
		if !container.Condition.Phase.Final() {
			r.log.Debug("condition is not final", "migration", migration.Name)
			continue
		}
		if err := r.cleanupContainerImage(migration, container.Name); err != nil {
			return err
		}
	}
	return nil
...
func (r *migrationReconciler) cleanupContainerImage(migration *v1.Migration, name string) error {
